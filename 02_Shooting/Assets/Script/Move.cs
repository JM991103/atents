...
public class Move : MonoBehaviour
{
    ...
    /// <summary>
    /// Update 함수, 매 프레임마다 호출괴는 함수, 지속적으로 변경되는 것이 있을 때 사용하는 함수
    /// </summary>
    private void Update()
    {
        ...
        //    //new Vector3(0, -1, 0);  // 아래쪽  Vector3.down;
           ...
           transform.position += (speed * Time.deltaTime * dir);   //아래쪽 방향으로 speed 만큼 움직여라(매 프레임마다)

        //    //Time.deltaTime : 이전 프레임에서 지금 프레임까지 걸린 시간
        //    //transform.position += (Vector3.down * speed * Time.deltaTime); //아래쪽 방향으로 speed만큼 움직여라 (매 초마다)
        //    //transform.position += (speed * Time.deltaTime * Vector3.down);
    }

    //    //Test_OidInputmanager();
    ...
    public void fire(InputAction.CallbackContext context)
    {
        ...
        /// 인풋 매니저 사용
        /// </summary>
        private void Test_OidInputmanager()
        {
        //Input Managet를 이용한 입력 처리
        //Busy Wait이 발생.(하는일이 없지만 사용되고 있는 상태 => 전력 세이빙을 방해 => 전력 커짐)
